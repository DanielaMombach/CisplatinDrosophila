...
EXPRESS=your_pathway/express-1.5.1-linux_x86_64/express
...
###Align reads to the reference genome with hisat2
	...
	"$HISAT2" -p 4 -x "$DIRREF"/"$REF""_hisat2.index" "$DIRFASTQ"/control_rep2.fq -S "$DIRALIGN"/control_rep2.sam
...
###Convert SAM files to BAM
	...
	echo -e "n*-----------control 2 SAM TO BAM"
        "$SAMTOOLS" view -bS "$DIRALIGN"/control_rep2 > "$DIRALIGN"/control_rep2.bam
...
###Count reads per gene with express
	...
	"$EXPRESS" -o "$DIRCOUNTREADS2"/control_rep2 -O 1 --output-align-prob --calc-covar "$DIRREF"/"$REF".fasta --f-stranded "$DIRALIGN"/control_rep2.bam

	"$EXPRESS" -o "$DIRCOUNTREADS2"/treatment_rep1 -O 1 --output-align-prob --calc-covar "$DIRREF"/"$REF".fasta --f-stranded "$DIRALIGN"/treatment_rep1.bam
